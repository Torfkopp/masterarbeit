...
#let slides(
  ...
  set page(
    ...
    header: [
        #v(-0.5em)
        #let primary-colour = white
        #let text-colour = title-color
        #if theme == "full" {
          primary-colour = title-color
          text-colour = white
        }
        #grid(
          columns: (50%, 50%),
          ...
          grid.cell(
            colspan: 2,
            components.simple-navigation(primary: text-colour, secondary: fill-color, background: primary-colour),
          ),
          // HEADING
          grid.cell(
            context {
              ...
              if heading != none {
                set align(top)
                set text(1.4em, weight: "bold", fill: text-colour)
  ...
  // FOOTER
    footer: [
        ...
        // Normal Styling of the Footer
          ...
          components.progress-bar(height: 2pt, title-color, body-color) //self.colors.primary, self.colors.neutral-lightest)
  ...
  if (title == none) {
    ...
    block(
      ...
      text(1.4em, fill: title-color, weight: "bold", authors.join(", ", last: " & ")) +
