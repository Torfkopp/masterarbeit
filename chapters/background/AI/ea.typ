...
Normally, the circle gets broken up when an individual's fitness reaches a predefined threshold.

Evolutionary algorithms encompass several subcategories, each inspired by different evolutionary theories:

- Darwinian Evolution: Rooted in Charles Darwin's theory of natural selection, this approach emphasises survival of the fittest. Genetic algorithms (GAs) are a primary example, where individuals are selected based on their fitness, and crossover and mutation create variation.

- Lamarckian Evolution: Based on Jean-Baptiste Lamarck's theory, which posits that traits acquired during an individual's lifetime can be passed to offspring. In EAs, this translates to individuals that can adapt and improve within their lifetime before passing on their enhanced traits, merging the ideas of learning and evolution.

- Swarm Intelligence: Inspired by the collective behaviour of social animals, such as birds flocking or fish schooling. Algorithms like Particle Swarm Optimisation (PSO) and Ant Colony Optimisation (ACO) fall under this category. These algorithms utilise simple agents that interact locally with one another and with their environment, leading to the emergence of complex, intelligent behaviour at the global level.
...
Evolutionary algorithms exemplify the power of bio-inspired computing, leveraging nature's principles of evolution to address complex computational challenges. Their ability to evolve solutions over generations, adapt to dynamic environments, and maintain diversity in the population underscores their unique advantage in solving optimisation problems.
References
